...
IFNULL(wipyi.holdCount,0) 'OHwip',
IFNULL(wipni.holdCount, 0) 'NOHwip',
...
LEFT JOIN cddb.alloccated a ON a.sku = cc.sku
LEFT JOIN products p ON p.sku = cc.sku
LEFT JOIN brands b ON b.brandID = p.brandID
LEFT JOIN inventoryStatus  is ON is.inventoryStatusID = cc.stat_snap
LEFT JOIN (SELECT 
	         sku,
             SUM(recQty) AS 'recQty',
             tsRecceived
           FROM `cddb`.`excceptions`
           WHERE tsRecceived > CURDATE()
